...
async def test_simple_ai_decision_with_timeout():
    """测试简单AI决策（带超时）"""
    console.print("[bold yellow]开始测试简单AI决策（带超时）[/bold yellow]")
    ...
    try:
        ...
        action = await asyncio.wait_for(
            asyncio.get_event_loop().run_in_executor(None, ai_agent.decide_action, game.players[1], game),
            timeout=10.0  # 10秒超时
        )
        ...
        end_time = asyncio.get_event_loop().time()
        elapsed_time = end_time - start_time
        console.print(f"[bold red]⏰ AI决策超时！[/bold red]")
        console.print(f"  耗时: {elapsed_time:.2f}秒")
...
def timeout_handler(signum, frame):
    """超时处理函数"""
    console.print("[bold red]⏰ 脚本执行超时，强制退出！[/bold red]")
    exit(1)
...
if __name__ == "__main__":
    ...
    # 运行测试
    asyncio.run(test_simple_ai_decision_with_timeout())
